...
export async function analyzeIngredients(
  ...
  parts.push({
    ...
    - 'imagePrompt': A detailed, literal visual prompt for image generation for THIS recipe only — same dish as title and ingredients; name the main components, cooking style (e.g. stir-fry, soup, baked), sauce/color, bowl vs plate, and garnish so an image model cannot confuse it with another recipe.
...
export async function generateRecipeImage(recipe: Recipe): Promise<string> {
  ...
  const visualBrief =
    ...
    recipe.appearance?.trim() ||
    "the finished plated dish as described in the recipe title and ingredients.";

  const prompt = `Food photograph for ONE specific dish only. The image must depict exactly this recipe — do not substitute a different meal or cuisine.

RECIPE TITLE (canonical): ${recipe.title}
CUISINE: ${recipe.cuisine}
CORE INGREDIENTS (must read as this dish): ${coreNames.length ? coreNames.join(", ") : "(see visual description)"}

VISUAL DESCRIPTION — follow this literally for plating, colors, textures, and garnishes:
${visualBrief}

Style: professional food photography, single plate or bowl, natural soft side-lighting, shallow depth of field, appetizing colors.
Constraints: no text, no letters, no watermarks, no people, no hands, no logos, no packaging.`;
